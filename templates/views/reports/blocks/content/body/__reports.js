...
/*/
 ...
 /*/ Перебераем массив с отчетами /*/
  ...
  $(reports).each(function(i, item){
      ...
      raport.find('.reports__body__line__assistant images').attr("src", item.assistant.avatar)
      raport.find('.reports__body__line__responsible span').text(item.responsible.name)
      raport.find('.reports__body__line__responsible images').attr("src", item.responsible.avatar)
